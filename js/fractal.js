function fractal(x_extent, y_extent) {
    ...
    var _menuItems = {
		...
		TricornItems: {
			name:"Tricorn",
			id:"TricornMenu",
			main:_methods.tricorn,
			others: []
		},
		BurningShipItems: {
			name:"Burning ship",
			id:"BurningShipMenu",
			main:_methods.burningShip,
			others: []
		},
		SiegelDiskItems: {
			name:"Siegel Disk",
			id:"SiegelDiskMenu",
			main:_methods.siegelDisk,
			others: []
		}
    }

    var __ = {
        method: _methods.julia,
        realMin: -1.7,
        realMax: 1.7,
        imagMin: -1,
        imagMax: 1,
    ...
    jul.render = function(){
        if(done){return true}
        if(resolution>3) jul.boxes()
        else return jul.renderAux()
    }

    jul.renderAux = function() {
