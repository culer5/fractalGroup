var width = 823; //document.getElementById("Content").clientWidth;
var height = 525; //document.getElementById("Content").clientHeight;
var canvas = document.getElementById('aCanvas');
var interaction_canvas = document.getElementById('bCanvas');
var CustColor;
...
var CustomColor = d3.scale.sqrt()
    .domain([0, jul.__.maxIter])
    .range(["black", CustColor])
    .interpolate(d3.interpolateLab);

var deep = d3.scale.linear()
    .domain(d3.range(0, jul.__.maxIter, 20))
    .range(d3.range(0, 100).map(function (i) {
        return d3.hcl(190 + 120 * i, 40 + 35 * Math.round(Math.cos(i / 5)), 18 + 6 * (i % 10)).toString();
    }))
    .interpolate(d3.interpolateLab);
...
function changeColor(color) {
    ...
    d3.select("#CustomColor").classed("active", color == CustColor);
}
...
window.onresize = function () {
    width = 700; //document.getElementById("Content").clientWidth;
    height = 525; //document.getElementById("Content").clientHeight;
